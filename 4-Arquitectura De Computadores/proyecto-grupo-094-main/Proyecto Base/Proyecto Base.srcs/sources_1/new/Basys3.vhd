...
signal disB              : std_logic_vector(15 downto 0); -- Se�ales de salida totalidad de los displays.
...
signal segundos         : std_logic_vector(15 downto 0);
signal milisegundos     : std_logic_vector(15 downto 0);
signal microsegundos    : std_logic_vector(15 downto 0);

signal muxin_to_ram     : std_logic_vector(15 downto 0);

signal botonesextendidos : std_logic_vector(15 downto 0);
...
inst_Timer: Timer port map( 
    ...
    clear  => clear,
    seconds  => segundos,
    mseconds => milisegundos,
    useconds => microsegundos    
    );

botonesextendidos <= "00000000000" & d_btn;
-- Mux IN
with ram_address select
    muxin_to_ram <= sw  when "000000000001",
                    botonesextendidos when "000000000011",
                    segundos when "000000000100",
                    microsegundos when "000000000101",
                    milisegundos when "000000000110",
...
-- DEMUXer
with ram_address select
        demux_out_1 <= write_ram when "000000000010",
                        '0'      when others;
with ram_address select
        demux_out_2 <= write_ram when "000000000000",
                        '0'      when others;
