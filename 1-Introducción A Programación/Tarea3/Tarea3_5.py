class DCCivilizacion:
    ...
    def invasion_ciudad(self, civilizacion_atacante, fila, columna):
        ...
        print(
            f'{civilizacion_atacante} ha atacado la ciudad de {ciudad_defensa.nombre}')
        ...
        if ciudad_defensa.vida <= 0:
            print(
                f'Se perdio la ciudad de {ciudad_defensa.nombre} a manos de {civilizacion_atacante}')
    ...
    def conquistar_ciudad(self, nombre_ciudad, tipo):
        ...
        for i in civilizacion.mapa:
            ...
            for e in i:
                if e == '':
                    ciudad = DCCiudad(nombre_ciudad, tipo,
                                      int(fila), int(columna))
                    civilizacion.agregar_ciudad(ciudad)
                    print(
                        f'{civilizacion.nombre} ha conquistado la ciudad de {ciudad.nombre}')
                    if tipo == 'financiera':
                        ...
                        print(
                            f'{ciudad.nombre} ha generado 10 puntos de avance tecnologico')
                    elif tipo == 'fortaleza':
                        civilizacion.proteccion += 15
                        print(
                            f'{ciudad.nombre} ha aportado con 15 puntos de proteccion')
                    return ciudad
                columna += 1
            fila += 1
...
for i in range(5):
    ...
    if evento[0] == 'Conquista':
        ...
        civilizacion.conquistar_ciudad(
            nombre_ciudad_conquistada, tipo_ciudad_conquistada)
